<template>
  <div class="app-container">
    ...
    <el-tabs v-model="tabsActiveName" type="card">
      ...
      <el-tab-pane label="按钮" name="tab2">
        <div class="text item">
          床头分机外接按钮设置：
          <div style="margin-top: 5px">
            <el-radio-group v-model="templateConfig.externalHotkey.buttonId">
              <el-radio
                v-for="dict in externalHotkeyOptions"
                :key="dict.value"
                :label="dict.value"
                >{{ dict.label }}</el-radio
              >
        ...
        <div class="text item">
          床头分机手持按钮设置：
          <div style="margin-top: 5px">
            <el-radio-group v-model="templateConfig.inHandHotkey.buttonId">
              <el-radio
                v-for="dict in inHandHotkeyOptions"
                :key="dict.value"
                :label="dict.value"
                >{{ dict.label }}</el-radio
              >
      ...
      <el-tab-pane label="功能列表" name="tab4"
        ><el-row>
          <el-col style="width: 150px">
            <el-card class="box-card-bed-template">
...
export default {
  ...
  data() {
    return {
      ...
      //模板id
      templateId: "0",
      templateUrl: require("@masterModule/assets/images/bed_style_0.jpg"),
      //模板选项
      themeOptions: [
        { value: 0, label: "默认风格" },
        { value: 1, label: "风格二" },
      ...
      //外接按钮设置
      externalHotkeyOptions: [
        { value: "0", label: "洗手间报警", emergencyCall: true },
        { value: "1", label: "普通呼叫", emergencyCall: false },
      ],
      //手持按钮设置
      inHandHotkeyOptions: [
        { value: "0", label: "换药提醒", emergencyCall: false },
        { value: "1", label: "床位清理", emergencyCall: false },
      ],
      //模板配置
      templateConfig: {
        //模板id
  ...
  methods: {
     ...
     */
    getTemplate() {
      getBedTemplate(this.templateId).then((res) => {
        if (res.code === 200) {
    ...
    handleStyleChange() {
      if (this.templateConfig.themeId === 0) {
        this.templateUrl = require("@masterModule/assets/images/bed_style_0.jpg");
      } else if (this.templateConfig.themeId === 1) {
        this.templateUrl = require("@masterModule/assets/images/bed_style_1.jpg");
      } else if (this.templateConfig.themeId === 2) {
        this.templateUrl = require("@masterModule/assets/images/bed_style_2.jpg");
      }
    },
    ...
    handleSubmit() {
      updateBedTemplate(this.templateConfig).then((res) => {
        if (res.code === 200) {
